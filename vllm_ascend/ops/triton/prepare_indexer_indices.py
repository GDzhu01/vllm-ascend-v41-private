...
def _prepare_indexer_indices_kernel(
    ...
    for block in range(first_block, last_block):
        ...
        if WRITE_LENGTHS:
            # MQSMLA consumes the number of valid compressed indices.  Count
            # them while the predicate is already resident in vector-core
            # registers instead of reconstructing it with GE/ReduceSum/Cast
            # launches at every attention consumer. Keep the reduction after
            # the sorted-index store: current Ascend Triton otherwise lets the
            # row-reduced layout leak into narrow (TopK <= 8) output stores.
...
def prepare_indexer_indices(
    ...
    *,
    ...
    # The in-kernel reduction wins decisively for decode and short prefill,
    # while monopolizing the sort kernel for long prefill. Select using the
    # amount of reduction work; the cutoff is measured on the A5 TopK=512
    # production profile. The external fallback still writes indices directly
    # into the graph-stable destination, so it does not restore the old Copy.
    write_lengths = lengths_output is not None and num_rows * reduction_cols <= _FUSED_LENGTHS_MAX_ELEMENTS
    if write_lengths:
        # Ascend Triton's row reduction layout is only reliable from one
        # 32-lane vector upward. Production uses TopK=512, while this floor
        # keeps the public helper correct for narrow standalone/test shapes.
