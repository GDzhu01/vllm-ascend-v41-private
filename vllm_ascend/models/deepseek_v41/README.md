...
`torch_npu.npu_rms_norm` directly with the checkpoint weight and epsilon.
Fused BF16 normalization can differ from the original intermediate rounding.
The standalone Triton compressor API retains its path.

The mHC reference projection and Engram gate call `torch_npu.npu_rms_norm`
directly with FP32 unit gamma and use its inverse-RMS output, preserving the
original projection and dot-product order. The production mHC pre path already
uses `npu_hc_pre_v2`.
