...
def _reference(
    ...
    image_mask = (input_ids >= IMAGE_SENTINEL_LO) & (
        input_ids < IMAGE_SENTINEL_LO + IMAGE_SENTINEL_COUNT
    )
...
def test_v41_vision_bias_and_image_sentinel(dtype: torch.dtype, with_hash: bool):
    ...
    actual_weights, actual_ids, _ = torch.ops._C_ascend.moe_gating_top_k_hash(
        x=logits.npu(),
        k=top_k,
        bias=text_bias.npu(),
        input_ids=input_ids.npu(),
        tid2eid=tid2eid.npu() if tid2eid is not None else None,
        k_group=1,
        group_count=1,
        routed_scaling_factor=1.5,
        eps=1e-20,
