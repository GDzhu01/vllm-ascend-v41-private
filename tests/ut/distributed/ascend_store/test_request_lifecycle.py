...
from vllm_ascend.core.circular_buffer import AscendCircularBufferSpec, prefix_cacheable
...
def cache_layout(request, speculative_method):
    ...
    ring = AscendCircularBufferSpec(block_size=32, num_kv_heads=1, head_size=16, dtype=torch.float32)
    ...
    def allocate():
        ...
        for group in plan.kv_cache_groups:
            for name in group.layer_names:
                ...
                rows = get_storage_block_size(spec)
...
def test_raw_sequence_lifecycle(
    ...
    has_private_state = any(not prefix_cacheable(group.kv_cache_spec) for group in plan.kv_cache_groups)
    config = create_vllm_config(
        ...
        kv_transfer_config=KVTransferConfig(
            ...
            kv_connector_extra_config={
                ...
                "consumer_is_to_put": kv_role == "kv_consumer",
                "consumer_is_to_load": kv_role == "kv_consumer",
            },
        ),
    )
    ...
    scheduler_plan = generate_scheduler_kv_cache_config([plan])
    config.cache_config.block_size = min(group.kv_cache_spec.block_size for group in scheduler_plan.kv_cache_groups)
    block_size, hash_size = resolve_kv_cache_block_sizes(scheduler_plan, config)
    ...
    try:
        ...
        assert scheduler.reset_prefix_cache()
        MemoryBackend.payloads = {}
        seed_length = 256 if speculative_method else 128
        run_request("local-prefix", producer_tokens[:seed_length], 0)
        MemoryBackend.payloads = dict(saved)
